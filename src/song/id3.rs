...
impl Text {
	...
	fn write(&self, mut w: impl Write) -> io::Result<usize> {
		let mut n = 0;
		// encoding:
		n += w.write(&[2u8])?;
		n += w.write(&self.0)?;
...
impl FrameType {
	...
	fn data_len(&self) -> u32 {
		match self {
			FrameType::Title(t) | FrameType::Artist(t) => t.len(),
			FrameType::Other { data, .. } => data.len() as u32,
		}
	}

	#[inline]
	fn write_data(&self, mut w: impl Write) -> io::Result<usize> {
		match self {
...
impl Frame {
	...
	fn read(mut r: impl Read) -> io::Result<Option<Self>> {
...
impl Id3 {
	pub fn read(mut source: impl Read + Seek) -> io::Result<Option<Self>> {
		...
		let flags = u8::read_be(&mut source)?;
		...
		if flags & (HeaderFlags::ExtendedHeader as u8) != 0 {
			let ext_header_size = u32::read_be(&mut source)?;
			log::warn!("Skipping extended header");
			source
	...
	pub fn as_bytes(&self) -> Vec<u8> {
		...
		let size: u32 = self.frames.iter().map(Frame::len).sum();
	...
	pub fn from_song(metadata: &SongMetadata) -> Self {
		Self {
			major_version: 3,
			revision: 0,
			flags: 0,
			frames: vec![
				Frame {
					flags: [0; 2],
					frame_type: FrameType::Title(Text::from(metadata.title.as_str())),
				},
				Frame {
					flags: [0; 2],
					frame_type: FrameType::Artist(Text::from(metadata.artist.as_str())),
				},
			],
		}
	}
